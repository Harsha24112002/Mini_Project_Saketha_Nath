...
for  c in range(0,100,10):
    mz = [10,13]
    xvz = np.array([[10,12],[12,13]])
    sz = xvz@xvz.T

    z = np.random.multivariate_normal(mz,sz,nz)
    ...
    for i in range(0,nz):
        x = np.vstack((x,np.random.multivariate_normal(mean1(z[i]), s, m)))
        y = np.vstack((y,np.random.multivariate_normal(mean2(z[i],c), s, m) ))
...
plt.savefig('Images/SameZMeanVary/f.png')

plt.figure(2)
plt.plot(c_arr,f1_arr)
plt.savefig('Images/SameZMeanVary/f1.png')


plt.figure(3)
plt.plot(c_arr,f2_arr)
plt.savefig('Images/SameZMeanVary/f2.png')


plt.figure(4)
plt.plot(c_arr,f3_arr)
plt.savefig('Images/SameZMeanVary/f3.png')
